...
module.exports = function(rawOptions) {
    ...
    _.forEach(mergedOptions.cssHandlebarsHelpers, function(helper, name) {
        if (typeof helper === 'function') {
            templater.registerHandlebarsHelper(name, helper)
        } else {
            throw new Error('custom css handlebas helpers can be actual function');
        }
    })
...
function normalizeTargetCss(mergedOptions) {
    ...
    if (!(css instanceof Array)) {
        css = [
            [css, mergedOptions.spritesheetTemplatesOptions]
        ];
    }
